...
// Struct for the verified bpf program info
struct bpf_verified {
    size_t prog_len;
    size_t reloc_len;
    struct bpf_relocation * relocs;
    __u8 * prog;
};
...
static int bpf_object__elf_collect(struct bpf_object *obj)
{
	...
	while ((scn = elf_nextscn(elf, scn)) != NULL) {
		...
		if (strcmp(name, "license") == 0) {
            ...
            } else if (strcmp(name, ".decouple") == 0) {
                pr_info("Loading in relocations and verified binary \n");
                obj->verified.reloc_len = *(__u32 *)(data->d_buf);
                obj->verified.relocs = calloc(obj->verified.reloc_len, sizeof(struct bpf_relocation));
                memcpy(obj->verified.relocs, data->d_buf + sizeof(__u32), obj->verified.reloc_len * sizeof(struct bpf_relocation));

                obj->verified.prog_len = data->d_size - (sizeof(__u32) + (obj->verified.reloc_len * sizeof(struct bpf_relocation)));
                obj->verified.prog = calloc(1, obj->verified.prog_len);
                memcpy(obj->verified.prog, data->d_buf + 4 + (obj->verified.reloc_len * sizeof(struct bpf_relocation)), obj->verified.prog_len);
...
struct bpf_object * bpf_object__open_verified(const char * path)
{
    ...
    const void * obj_buf = NULL;
    size_t obj_buf_sz = 0;
    const struct bpf_object_open_opts * opts = NULL;
...
// Load the object treating it as a pre-verified object
int bpf_object__load_verified(struct bpf_object * obj)
{
    char * path;
    union bpf_attr attr;
    ...
    attr.relocations = (__aligned_u64)obj->verified.relocs;
    attr.relocations_length = obj->verified.reloc_len;
    attr.blob_prog_type = obj->programs->type;

    return syscall(__NR_bpf, BPF_PROG_LOAD_VERIFIED, &attr, sizeof(attr));
}
